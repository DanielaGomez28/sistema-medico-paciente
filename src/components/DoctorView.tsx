...
import { accountNumber, formatPhone, onlyLetters, safeText } from '../lib/inputValidation';
...
export default function DoctorView({ doctorName, doctorEmail, doctorId, doctorProfile, onLogout }: DoctorViewProps) {
  ...
  useEffect(() => {
    ...
    const loadBankProfile = async () => {
      ...
      try {
        ...
        if (profile.cedula) {
          setDoctorCedula(profile.cedula);
        } else if (profile.bankHolderId) {
          setDoctorCedula(profile.bankHolderId);
        }
  ...
  const handleConfirmDoctorProfileEdit = async () => {
    ...
    if (!doctorPhoneIsValid || !bankHolderIsValid || !bankEntityIsValid || !accountNumberIsValid || !bankMobileIsValid || !officeAddressIsValid || !officeMunicipioIsValid) {
